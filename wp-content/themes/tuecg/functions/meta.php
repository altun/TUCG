...
$seo_portfolio = array(
	...
	array(
			'name' => 'Custom SEO Description',
			'desc' => 'Enter your Custom Page Description',
			'id' => 'ls_SEO_Description',
			'type' => 'text',
			'std' => ''
		),
        
 	array(
			'name' => 'Custom SEO Keywords',
			'desc' => 'Enter your Custom Title Description',
			'id' => 'ls_SEO_Keywords',
			'type' => 'text',
			'std' => ''
		),
...
function ls_add_box() {
    ...
    //////////////////////////////
	add_meta_box($seo_portfolio['id'], $seo_portfolio['title'], 'ls_show_box_seo_portfolio', $seo_portfolio['page'], $seo_portfolio['context'], $seo_portfolio['priority']);
    add_meta_box($seo_post['id'], $seo_post['title'], 'ls_show_box_seo_post', $seo_post['page'], $seo_post['context'], $seo_post['priority']);    
    add_meta_box($seo_page['id'], $seo_page['title'], 'ls_show_box_seo_page', $seo_page['page'], $seo_page['context'], $seo_page['priority']);
...
function ls_show_box_seo_portfolio() {
	global $seo_portfolio, $post;
 	
	echo '<input type="hidden" name="ls_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
 
	echo '<table class="form-table">';
 
	foreach ($seo_portfolio['fields'] as $field) {
...
function ls_show_box_seo_post() {
	global $seo_post, $post;
 	
	echo '<input type="hidden" name="ls_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
 
	echo '<table class="form-table">';
 
	foreach ($seo_post['fields'] as $field) {
...
function ls_show_box_seo_page() {
	global $seo_page, $post;
 	
	echo '<input type="hidden" name="ls_meta_box_nonce" value="', wp_create_nonce(basename(__FILE__)), '" />';
 
	echo '<table class="form-table">';
 
	foreach ($seo_page['fields'] as $field) {
...
function ls_save_data($post_id) {
    $field = array();
    $new = '';
	global $seo_portfolio, $seo_post, $seo_page, $meta_box_video, $meta_box_select_page;
