...
function startis_title( $atts,$content = null ) {
    return '<div class="stitle"><h3><span>' . $content . '</span></h3></div>';
}
...
function startis_testimonial($atts, $content = null) {
    ...
    $ticlass = '';
    if ($customer_image != '') {
        $image = vt_resize('',$customer_image , 50, 50, true ); 
        $ticlass = ' wim';
        if (isset($image)) {
...
function fa_icons($atts, $content = null) {
    ...
    if($url){ 
	return '<a href="'.$url.'" class="faicon'.$align.$class.'"><i '.$color.' class="'.$name.$size.$align.$circle.'"></i><span>'.$output.'</span></a>';
    } else {
    return '<i '.$color.' class="'.$name.$size.$class.$align.$circle.'">'.$output.'</i>';    
    }    
}
...
function startis_styled_boxes($atts, $content = null, $code) {
    ...
    if ($code=='warning') {
        $ico = '<i class="icon-warning-sign icon-3x"></i>';
    } elseif ($code=='error') {
        $ico = '<i class="icon-remove-sign icon-3x"></i>';
    } elseif ($code=='success') {
        $ico = '<i class="icon-flag icon-3x"></i>';
    } elseif ($code=='info') {
        $ico = '<i class="icon-info-sign icon-3x"></i>';
    } elseif ($code=='note') {
        $ico = '<i class="icon-paper-clip icon-3x"></i>';
    }
    
	$align = $align?' align'.$align:'';
    $close = $close?'<i class="icon-remove boxclose"></i>':'';
...
function startis_read_more( $atts ) {
    ...
    $align = $align?' align'.$align:'';
    $html = '<a class="read-more'.$align.'" href="'.$url.'" title="'.$title.'"'.$target.'><span>'.do_shortcode($text).'</span> <i class="icon-double-angle-right"></i></a>';
    return $html;
}
...
function startis_toggle($atts, $content = null, $code) {
	...
	return '<div class="toggle"><h3 class="toggle_title"><span class="toggle_plus"></span>' . $title . '</h3><div class="toggle_content">' . do_shortcode(del_p(trim($content))) . '</div></div>';
}
...
function startis_tabs($atts, $content = null, $code) {
	...
	if (!preg_match_all("/(.?)\[(tab)\b(.*?)(?:(\/))?\](?:(.+?)\[\/tab\])?(.?)/s", $content, $matches)) {
		...
		for($i = 0; $i < count($matches[0]); $i++) {
		    if (@$matches[3][$i]['icon']) { $icon = '<i class="'.@$matches[3][$i]['icon'].' alignleft"></i>'; }
...
function startis_pb_tabs($atts, $content = null, $code) {
	...
	if (!preg_match_all("/(.?)\[(pb_tab)\b(.*?)(?:(\/))?\](?:(.+?)\[\/pb_tab\])?(.?)/s", $content, $matches)) {
	   ...
	   //var_dump($matches);
		...
		for($i = 0; $i < count($matches[0]); $i++) {
		    if (@$matches[3][$i]['icon']) { $icon = '<i class="'.@$matches[3][$i]['icon'].' alignleft"></i>'; $iconlabel = 'class="withicon"'; }
...
add_shortcode('slide', 'startis_content_slide');
	...
	extract(shortcode_atts(array(
        ...
        if ($title) { if ($url) { $output .= '<span class="slidetitle"><a href="'.$url.'">'.$title.'</a></span>'; } else { $output .= '<span class="slidetitle">'.$title.'</span>'; } }
...
function startis_recent_posts($atts) {
	...
	if ($qwr->have_posts()){
		...
		$output .= '<ul class="posts_list fromblog">';
		while ($qwr->have_posts()){
			...
			global $post;
                ...
                $excerpt = strip_shortcodes($excerpt);
                        ...
                        $output .= '<span><i class="icon-calendar"></i> <strong>'. get_the_time( get_option('date_format') ).'</strong></span>';
                            if ($post->comment_count>0) $output .= '<span><i class="icon-comments-alt"></i> '. $post->comment_count.'</span>';
                ...
                if($moretag && strpos($excerpt, '<!--more-->') ){  
                    ...
                    $excerpt = "". str_replace( "\n", "<br />", $excerpt ) ."";  
                    $readmorelink  = ($readmorelink=='true') ? ' <a class="read-more" href="'.get_permalink().'" >'.$readmoretext.' <i class="icon-double-angle-right"></i></a> ' : '';
...
function startis_blog_posts($atts) {
	...
	if ($qwr->have_posts()){
		...
		while ($qwr->have_posts()){
            ...
            if ($ii==1) { $output .= '<ul class="posts_list fromblog type-post">'; }
			...
			if($thumbnail!='false'){
			     if (has_post_thumbnail() ){
                    ...
                    if ($ii==0) { $image = vt_resize(null, $imgsrc[0], 285, 190, true ); } else {
		...
		while ($qwr->have_posts()){
			...
			if($thumbnail!='false'){
			     if (has_post_thumbnail() ){
                    ...
                    $output .=  '<img src="'. $image['url'].'" alt="'.get_the_title().'" title="'.get_the_title().'"/>';
                                            $output .= '<div class="carousel_post_info"><span class="h6">'.get_the_title().'</span><span><i class="icon-calendar"></i>'. get_the_time( get_option('date_format') ).'</span>';
                            if (@$post->comment_count>0) $output .= '<span><i class="icon-comments-alt"></i> '. @$post->comment_count.'</span>';
				    }
				    $output .= '</div></a>';
                }
			}
			$output .= '<a href="'.get_permalink().'"><div class="overlay_fx"><span class="bgoverlay"></a></span>';
			$output .= '<h6><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></h6>';
			...
			global $post;
                ...
                if($moretag && strpos($excerpt, '<!--more-->') ){  
                    ...
                    $excerpt = preg_replace( "!\n\n+!s", "</p><p>", $excerpt );  
                    $excerpt = "". str_replace( "\n", "<br />", $excerpt ) ."";  
                    $readmorelink  = ($readmorelink=='true') ? ' <a href="'.get_permalink().'" >'.$readmoretext.' <i class="icon-double-angle-right"></i></a> ' : '';
				    $output .= '<p>'.$excerpt.' '.$readmorelink.'</p>';                
                }  else { 
                    $readmorelink  = ($readmorelink=='true') ? ' <a href="'.get_permalink().'" >'.$readmoretext.' <i class="icon-double-angle-right"></i></a> ' : '';
    				if (iconv_strlen($excerpt, 'utf-8') > $desc_length ){  
...
function startis_portfolio_items_carousel($atts) {
	extract(shortcode_atts(array(
		'count' => '4',
		'category_name' => '',
        'thumb_width' => '220',
	...
	if ($qwr->have_posts()){
		$output = '<div class="rworks es-carousel-wrapper" data-width="'.$thumb_width.'" data-height="'.$thumb_height.'"><div class="clearfix">';
		...
		while ($qwr->have_posts()){
            ...
            if ($ls_embed_code!='') {
                $output .= '<div class="overlay_fx"><span class="bgoverlay"><span class="coverlay"></span><a href="'.get_permalink().'"><i class="icon-link"></i></a> <a title="'.get_the_title().'" data-rel="magnific" href="'.$ls_embed_code.'"><i class="icon-play-sign"></i></a> </span>';
            } else {
                $output .= '<div class="overlay_fx"><span class="bgoverlay"><span class="coverlay"></span><a href="'.get_permalink().'"><i class="icon-link"></i></a> <a title="'.get_the_title().'" data-rel="magnific" href="'.$imgsrc[0].'"><i class="icon-zoom-in"></i></a> </span>';
            }
			$output .= '<h6><a href="'.get_permalink().'" title="'.get_the_title().'">'.get_the_title().'</a></h6>';
			$output .= '</div>';
			$output .= '</li>';
...
function startis_lightbox($atts) {
    ...
    $align = $align?' align'.$align:'';
    $url = $url?' <a href="'.$url.'"><i class="icon-link"></i></a>':'';
	$image = vt_resize('',$src , $width, $height, true ); 
    if (isset($image)) {
        $retina = $retina?' '.$image['class']:'';
	...
	$output = '<div class="lightboximages" ><span class="portfolio_item"><a  data-rel="magnific" style="position: relative;" title="'.$title.'" href="'.$src.'" class="'.$align.'">'.$thumb.'</a><span class="bgoverlay"><span class="coverlay"></span>'.$url.'<a title="'.$title.'" data-rel="magnific" href="'.$src.'"><i class="icon-zoom-in"></i></a></span></span></div>';
	return $output;
}
...
if (isset($image)) {
return 
'<li data-width="'.$width.'">'.($client?'<i class="icon-info-sign"></i>':'').'
